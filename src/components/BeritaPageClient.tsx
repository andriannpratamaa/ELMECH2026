...
export default function BeritaPageClient({
  items,
  trending,
}: BeritaPageClientProps) {
  const [search, setSearch] = useState("");
  const [filterTag, setFilterTag] = useState<string | null>(null);
  const [page, setPage] = useState(1);

  const NEWS_ITEMS = items ?? [];
  const trendingItems = trending ?? NEWS_ITEMS.slice(0, 3);
  const tags = useMemo(
    () => [...new Set(NEWS_ITEMS.map((item) => item.tag))],
    [NEWS_ITEMS],
  );

  const headline = NEWS_ITEMS[0] ?? null;
  const restItems = NEWS_ITEMS.slice(1);

  const filtered = restItems.filter((item) => {
    const matchSearch =
      !search ||
          ...
          {headline && (
            <Link href={`/berita/${headline.slug}`}>
              ...
              >
                ...
                <div
                  className={`absolute inset-0 bg-gradient-to-br ${headline.color} opacity-20 mix-blend-multiply`}
                />
                <div className="absolute top-6 left-6">
                  <span
                    className={`inline-block px-3 py-1.5 rounded-xl text-xs font-semibold ${tagColors[headline.tag] || "bg-gray-100 text-gray-700"}`}
                  >
                    {headline.tag}
                  </span>
                </div>
