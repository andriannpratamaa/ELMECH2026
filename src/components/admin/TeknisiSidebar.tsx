...
import { useTeknisiNotification } from "@/contexts/TeknisiNotificationContext";
...
export default function KalabSidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const { pendingLabCount } = useTeknisiNotification();
  ...
  const isActive = (href: string) => {
    if (href === "/teknisi/dashboard") return pathname === href;
  ...
  const sidebar = (
    <aside className="h-full flex flex-col bg-[#0F172A] border-r border-white/5">
      <div className="flex items-center justify-between h-16 px-5 border-b border-white/5">
        <Link href="/teknisi/dashboard" className="flex items-center gap-2.5">
          ...
          <span className="text-sm font-bold text-white font-[family-name:var(--font-display)]">PLP Panel</span>
        </Link>
              ...
              {group.items.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {item.href === "/teknisi/labs" && pendingLabCount > 0 && (
