        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <button onClick={() => setShowMenu(!showMenu)} className="flex items-center gap-2 p-1.5 rounded-xl hover:bg-white/5 transition-colors">
              ...
              <span className="hidden sm:block text-sm text-white/70">{user?.name || "Teknisi"}</span>
            </button>
            ...
            {showMenu && (
              <>
                ...
                >
                  <button onClick={() => { setShowMenu(false); router.push("/teknisi/dashboard"); }} className="w-full text-left px-4 py-2 text-sm text-white/70 hover:text-white hover:bg-white/5 transition-colors">
                    Dashboard
                  </button>
