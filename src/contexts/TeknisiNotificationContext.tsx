...
type TeknisiNotificationContextType = {
  pendingLabCount: number;
  refreshNotifications: () => Promise<void>;
};

const TeknisiNotificationContext =
  createContext<TeknisiNotificationContextType | null>(null);

export function TeknisiNotificationProvider({
  children,
}: {
  ...
  const refreshNotifications = async () => {
    ...
    if (!user || user.role !== "teknisi") {
      ...
      const myLabs = user?.id
        ? labsData.filter((lab) => lab.teknisi_id === user.id)
        : [];
      ...
      console.error("Gagal memuat notifikasi teknisi", err);
    }
  };
  ...
  return (
    <TeknisiNotificationContext.Provider
      value={{
        pendingLabCount,
        refreshNotifications,
      }}
...
export function useTeknisiNotification() {
  const context = useContext(TeknisiNotificationContext);

  if (!context) {
    throw new Error("TeknisiNotificationProvider belum dipasang");
  }
