...
import type { Lab, Item, Profile } from "@/types/admin";
...
export default function KalabLabDetailPage() {
    ...
    const displayItems = readOnly
        ? labItems.filter((i) => itemStatuses[i.id]?.exists)
        : labItems;
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            if (found) {
                ...
                switch (u?.role) {
                    case "kalab":
                        hasAccess = found.kalab_id === u.id;
                        break;

                    case "plp":
                        hasAccess = found.plp_id === u.id;
                        break;

                    case "teknisi":
                        hasAccess = found.teknisi_id === u.id;
                        break;
                }

                if (!hasAccess) {
                    toast.error("Anda tidak memiliki akses ke laboratorium ini");
                    router.push(`/${u?.role}/labs`);
                    return;
                }
        ...
        },
        {
            ...
            render: (i: Item) => (
                <div className="flex items-center gap-2">
                    <button
                        onClick={() => router.push(`/plp/labs/${id}/inspeksi/${i.id}?tahun=${selectedSemester?.tahun}&semester=${selectedSemester?.semester}`)}
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center gap-3">
                <Link href="/plp/labs" className="p-2 rounded-xl hover:bg-white/5 text-white/50 hover:text-white transition-colors">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm font-medium text-white flex items-center gap-1.5">
                                    ...
                                    {lab.plp_name || (lab.plp_id ? `User #${lab.plp_id}` : "—")}
                                </p>
                            </div>
                            <div>
