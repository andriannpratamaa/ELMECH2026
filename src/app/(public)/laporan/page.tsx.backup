...
import { FileText, Download, Search, FileSpreadsheet, Shield, FileCheck } from 'lucide-react';
  ...
  return (
    <>
      ...
      <section className="relative pb-20 sm:pb-28 bg-white overflow-hidden">
        <div className="absolute w-[400px] h-[400px] rounded-full blur-[120px] opacity-[0.03] pointer-events-none" style={{ background: '#1E3A8A', top: '20%', left: -50 }} />
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <SectionTitle badge="Download Center" title="Dokumen" highlight="Tersedia" align="left" />
          ...
          <motion.div
            ...
            className="flex flex-col sm:flex-row gap-4 p-5 rounded-2xl bg-[#F8FAFC] border border-gray-100 mb-12"
          >
            ...
            <div className="flex gap-2">
              <button onClick={() => setFilterFormat(null)}
                className={`px-3 py-2 rounded-xl text-xs font-medium transition-all ${!filterFormat ? 'bg-[#0F172A] text-white shadow-sm' : 'bg-white text-gray-600 hover:bg-gray-100 border border-gray-200'}`}
              >Semua Format</button>
              {formats.map((fmt) => (
                <button key={fmt} onClick={() => setFilterFormat(fmt === filterFormat ? null : fmt)}
                  className={`px-3 py-2 rounded-xl text-xs font-medium transition-all ${filterFormat === fmt ? 'bg-[#0F172A] text-white shadow-sm' : 'bg-white text-gray-600 hover:bg-gray-100 border border-gray-200'}`}
                >{fmt}</button>
              ))}
            </div>
          </motion.div>
          ...
          {filtered.length === 0 ? (
            <EmptyState message="Tidak ada dokumen ditemukan" />
          ) : (
              ...
              {filtered.map((report, i) => (
                ...
                >
                  ...
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-orange-100 to-orange-50 flex items-center justify-center mb-5 group-hover:scale-110 transition-transform duration-300">
                    {report.format === 'XLSX' ? (
                      <FileSpreadsheet className="w-7 h-7 text-emerald-600" strokeWidth={1.5} />
                    ) : (
                      <FileText className="w-7 h-7 text-[#F97316]" strokeWidth={1.5} />
                    )}
                  </div>
                  <h3 className="text-base font-bold text-gray-900 mb-2 font-[family-name:var(--font-display)]">{report.title}</h3>
                  <p className="text-sm text-gray-500 leading-relaxed mb-5">{report.desc}</p>
                  <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                    <span className="text-xs text-gray-400">{report.format} &bull; {report.fileSize}</span>
