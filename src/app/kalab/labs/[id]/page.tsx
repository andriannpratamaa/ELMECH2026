...
export default function KalabLabDetailPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (found) {
        if (u?.laboratory_id && found.id !== u.laboratory_id) {
    ...
    { key: "tanggal_pembelian", header: "Tgl Pembelian", render: (i: Item) => <span className="text-white/50">{formatDate(i.tanggal_pembelian)}</span> },
    {
      ...
      render: (i: Item) => (
        <div className="flex items-center gap-2">
          <button
            onClick={() => router.push(`/kalab/labs/${id}/inspeksi/${i.id}`)}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Link href="/kalab/labs" className="p-2 rounded-xl hover:bg-white/5 text-white/50 hover:text-white transition-colors">
